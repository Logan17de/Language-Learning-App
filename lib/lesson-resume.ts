...
export function restartIncompleteLessonPhase(session: LessonSession): LessonSession {
  ...
  return {
    ...
    grammarAnswers: resetFrom <= 2 ? [] : session.grammarAnswers,
    readingAnswers: resetFrom <= 3 ? [] : session.readingAnswers,
    readingEvents: resetFrom <= 3 ? [] : session.readingEvents,
    readingComplete: resetFrom <= 3 ? false : session.readingComplete,
    listeningEvents: resetFrom <= 4 ? [] : session.listeningEvents,
    listeningComplete: resetFrom <= 4 ? false : session.listeningComplete,
    speakingEvents: resetFrom <= 5 ? [] : session.speakingEvents,
    speakingComplete: resetFrom <= 5 ? false : session.speakingComplete,
    completionResult: null,
