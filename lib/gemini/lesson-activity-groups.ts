...
function adaptGrammarTeaching(
  ...
  return targets.map((target, index) => {
    ...
    return authoritativeGrammarTeaching({
...
/**
 * Reapply the committed grammar bank at the final lesson boundary.
 *
 * Activity groups are durable checkpoints. A group generated by an older
 * deployment can be resumed and assembled by a newer deployment, so curating
 * only when the group is first generated is not enough. Running the overlay
 * again during assembly guarantees that both fresh and resumed jobs persist the
 * exact Structure, When to use it, and Example fields from the bank.
 */
export function authoritativeGrammarTeaching(
  item: GrammarTeachingItem,
): GrammarTeachingItem {
  const curated = curatedGrammarPattern(item.pattern);
  return {
    ...item,
...
export function assemblePlayableLesson(input: {
  ...
  return {
    ...
    grammar: input.groups.grammarAndReading.grammarTeaching.map((rawItem) => {
      const item = authoritativeGrammarTeaching(rawItem);
      return {
        libraryId: item.libraryId,
