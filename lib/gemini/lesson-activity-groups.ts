...
import { activityGroupCheckpointIssues } from "@/lib/custom-lessons/checkpoint-validation";
...
function libraryIds(library: ResolvedLessonLibrary): Set<string> {
  return new Set([
...
function reviewIssues(
  ...
  const issues = activityGroupCheckpointIssues(
    "final_review",
    value,
    libraryIds(library),
  );
  if (!isRecord(value) || !Array.isArray(value.reviewQuestions)) {
    return ["Review response must contain exactly 5 questions.", ...issues];
  }
  if (value.reviewQuestions.length !== 5) {
    return ["Review response must contain exactly 5 questions.", ...issues];
  }
  const categories = value.reviewQuestions.map((question) =>
    isRecord(question) ? question.category : null,
  );
  const required = [
    "kanji",
    "vocabulary",
  ...
  const categoryIssues = required.every(
    (category) => categories.filter((value) => value === category).length === 1,
  )
    ? []
    : [
        "Review response must contain exactly one kanji, vocabulary, grammar, listening, and speaking question.",
      ];
  return [...categoryIssues, ...issues];
}
...
function context(
  ...
  include: Array<"kanji" | "vocabulary" | "grammar">,
...
function targetIdForVocabularyQuestion(
  ...
  const contextualVocabulary = inputLibraryVocabularyMatch(
    library,
    evidence,
  );
...
function inputLibraryVocabularyMatch(
  library: ResolvedLessonLibrary,
  evidence: string,
) {
  return library.vocabulary.find((item) =>
    [item.term, item.reading, item.meaning]
      .filter(Boolean)
...
function rawVocabularyQuestionIssues(
  ...
  value.questions.forEach((raw, index) => {
    if (!isRecord(raw)) {
      issues.push(`Vocabulary question ${index + 1} must be an object.`);
      return;
    }
    const question = raw as unknown as RawVocabularyQuestion;
    const choices = Array.isArray(question.choices)
      ? question.choices.map((choice) => normalized(choice))
      : [];
    if (choices.length !== 4 || new Set(choices).size !== 4) {
      issues.push(`Vocabulary question ${index + 1} needs four distinct choices.`);
    }
    if (!choices.includes(normalized(question.answer ?? ""))) {
      issues.push(`Vocabulary question ${index + 1} answer must occur in its choices.`);
    }
    if (!targetIdForVocabularyQuestion(question, library)) {
      issues.push(`Vocabulary question ${index + 1} does not resolve to a relevant library target.`);
    }
  });
  return issues;
}
...
function adaptVocabularyQuestions(
  ...
  return {
    vocabularyQuestions: questions.map((question) => {
      ...
      if (!targetId) {
        throw new Error("Vocabulary question does not reference a valid library target.");
      }
      return {
...
function storyGrammarTargets(
  ...
  return library.grammar.filter((item) => allowedPatterns.has(item.pattern));
}
...
function rawGrammarQuestionIssues(
  ...
  const issues: string[] = [];
  if (value.questions.length !== 10) {
    issues.push("Grammar response must contain exactly 10 questions.");
  }
  issues.push(...difficultySplitIssues(
    value.questions,
  ...
  value.questions.forEach((raw, index) => {
    if (!isRecord(raw)) {
      issues.push(`Grammar question ${index + 1} must be an object.`);
      return;
    }
    const question = raw as unknown as RawGrammarQuestion;
    const choices = Array.isArray(question.choices)
      ? question.choices.map((choice) => normalized(choice))
      : [];
    if (choices.length !== 4 || new Set(choices).size !== 4) {
      issues.push(`Grammar question ${index + 1} needs four distinct choices.`);
    }
    if (!choices.includes(normalized(question.answer ?? ""))) {
      issues.push(`Grammar question ${index + 1} answer must occur in its choices.`);
    }
    if (grammarTargetsForQuestion(question, targets).length < 1) {
      issues.push(`Grammar question ${index + 1} does not resolve to a relevant library target.`);
    }
  });
  return issues;
}
...
function adaptGrammarQuestions(
  ...
  return questions.map((question) => {
    ...
    if (!target) {
      throw new Error("Grammar question does not reference a valid library target.");
    }
    return {
      activityType: "multiple_choice",
      difficulty: rawDifficulty(question.difficulty),
...
export async function generateFinalReviewActivities(input: {
  ...
  const generated = await generateStructured<ReviewGroup>({
    name: "final_review",
    prompt: [
...
function inspectableTerms(
  ...
  const coveredCharacters = new Set(
    vocabulary.flatMap(
      (item) => item.surface.match(/\p{Script=Han}/gu) ?? [],
    ),
  );
...
function storyWords(
  ...
  return line.terms.map((term) => {
    const exact = library.vocabulary.find(
      (item) =>
        item.term === term.surface && item.reading === term.readingHint,
    );
...
export function assemblePlayableLesson(input: {
  ...
  const packageKanji =
    targetKanji.length > 0
      ? targetKanji.flatMap((character) => {
          const item = input.library.kanji.find(
  ...
  return {
    ...
    vocabularyQuestions:
      input.groups.vocabularyAndKanji.vocabularyQuestions.map((question) =>
        withTerms(question, "vocabulary"),
      ),
    ...
    speakingExercises: input.groups.communication.speakingExercises.map(
      (exercise) => ({
        ...
        inspectableTerms:
          exercise.inspectableTerms ??
          inspectableTerms(
            [exercise.prompt, exercise.modelAnswer],
            input.library,
          ),
      }),
    ),
