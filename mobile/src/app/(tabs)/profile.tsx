...
export default function ProfileScreen() {
  const router = useRouter(); const { signOut } = useSession(); const [profile, setProfile] = useState<Profile | null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null);
  ...
  return <Screen><Brand /><View style={styles.intro}><Eyebrow>Profile</Eyebrow><Heading>{profile?.display_name ?? 'Your AIko account'}</Heading><Copy>{profile?.email}</Copy></View>{error && <InlineNotice tone="danger">{error}</InlineNotice>}
    ...
    <Button label="Plan and access" variant="secondary" icon="diamond-outline" onPress={() => router.push('/subscription')} /><Button label="Sign out" variant="quiet" icon="log-out-outline" onPress={() => { void signOut(); }} />
  </Screen>;
}
