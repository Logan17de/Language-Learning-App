...
export function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (mode === "signup" && !isStrongEnough(password)) {
      setError("Choose a stronger password using at least three of the four requirements.");
      return;
    }
    setLoading(true);
    if (backendMode === "demo") {
  ...
  async function googleSignIn() {
    setError("");
    if (backendMode === "demo") {
      setError("Google sign-in becomes available when Supabase Auth is connected.");
      return;
    }
    setLoading(true);
  ...
  return (
    <form className="mt-8 space-y-5" onSubmit={submit}>
      <Button type="button" variant="secondary" className="w-full" disabled={loading} onClick={googleSignIn}>
        <GoogleMark />
      ...
      <label className="block">
        ...
        <span className="relative block">
          <input required minLength={mode === "signup" ? 8 : 6} value={password} onChange={(event) => setPassword(event.target.value)} type={showPassword ? "text" : "password"} className="form-input pr-12" placeholder={mode === "signup" ? "Create a strong password" : "Enter your password"} autoComplete={mode === "signup" ? "new-password" : "current-password"} />
      ...
      <p className="text-center text-xs leading-5 text-stone-400">
        {backendMode === "demo" ? "Demo mode: email sign-in is local; Google requires Supabase Auth." : "Your account is secured by Supabase Auth."}
      </p>
    </form>
  );
}
