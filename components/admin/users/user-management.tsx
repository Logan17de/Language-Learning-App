...
export function UserManagement() {
  ...
  const users: AdminUserRecord[] = backendRows.map(
    ({ profile, lessonsCompleted, supportRequestCount, reportCount, lastActive }) => ({
      id: profile.id,
  ...
  async function updateUser(user: AdminUserRecord, action: string) {
    ...
    setBackendRows((items) =>
      items.map((item) =>
        item.profile.id === user.id
          ? { ...item, profile: { ...item.profile, status: user.status } }
          : item,
      ),
    );
    setMessage(action);
  ...
  async function changePlan(userId: string, nextPlan: SubscriptionPlan) {
    ...
    const response = await fetch(`/api/admin/subscriptions/${userId}`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        plan: nextPlan === "premium" ? "premium_monthly" : "free",
        status: "active",
      }),
    });
    const body = (await response.json().catch(() => null)) as
        ...
        item.profile.id === userId
          ? {
              ...
              profile: {
                ...item.profile,
                subscription_plan:
                  nextPlan === "premium" ? "premium_monthly" : "free",
              },
            }
          : item,
      ),
  ...
  return (
    <>
      <AdminPageHeader
        ...
        description="Live learner accounts with recent activity, completion, report, support, subscription, and account-state controls."
        actions={
          <Button
            type="button"
            variant="secondary"
        ...
        <AdminTable
          ...
          rows={visible.map((user) => ({
            ...
            cells: [
              ...
              <select
                key="plan"
                aria-label={`Plan for ${user.displayName}`}
              ...
              <div key="actions" className="flex min-w-max gap-1">
                ...
                <Icon
                  ...
                  onClick={() => setConfirm({ action: "reset", user })}
                >
                  <RotateCcw className="size-4" />
                </Icon>
                <Icon
                  label="Export user summary"
                  onClick={() => exportUser(user)}
                >
                  <Download className="size-4" />
                </Icon>
                <Icon
                  label="Mark account deleted"
      ...
      <AdminConfirmDialog
        ...
        description={
          confirm?.action === "delete"
            ? `${confirm?.user.displayName} will be marked deleted. This does not delete the Supabase Auth identity.`
            : `${confirm?.user.displayName}'s persisted learning progress will be reset.`
        }
        confirmLabel={confirm?.action === "delete" ? "Mark deleted" : "Reset progress"}
