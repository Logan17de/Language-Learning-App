...
export function GeneratedValidationDetail({ lessonId }: { lessonId: string }) {
  ...
  async function decide(decision: "approved" | "rejected") {
    ...
    setMessage(
      decision === "approved"
        ? "Generated lesson approved."
        : "Generated lesson rejected.",
    );
    setNote("");
    await load();
    setWorking(false);
  ...
  async function publish() {
    ...
    const response = await fetch(`/api/admin/lessons/${lessonId}/publish`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        changeSummary: note || "Published after generated-lesson review",
      }),
    });
    const body = (await response.json().catch(() => null)) as
      | { error?: string }
  ...
  if (loading && !data) {
    return (
      <div
        ...
        aria-label="Loading generated lesson validation"
      />
    );
  }
  if (!data) {
    return (
      <AdminEmptyState
        title="Generated lesson could not be loaded"
        description={
          error || "No stored generated job is linked to this lesson."
        }
      />
    );
  }
  ...
  const categories = Array.from(
    new Set(data.checks.map((check) => check.category)),
  );
    ...
    <>
      <AdminPageHeader
        ...
        title={
          lesson?.title ||
          data.request?.topic ||
          `Generated lesson ${lessonId.slice(0, 8)}`
        }
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <SummaryCard
          ...
          value={currentStatus.replaceAll("_", " ")}
          status
        />
        <SummaryCard
          label="Validation score"
          value={validation ? `${validation.score}/100` : "Not run"}
        />
        <SummaryCard label="Warnings" value={warningCount.toString()} />
        <SummaryCard
          label="Generation time"
          value={
            data.job.generation_seconds
              ? `${data.job.generation_seconds.toFixed(1)}s`
              : "Not recorded"
          }
        />
      </div>
      ...
      {data.job.error_message && (
        <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 p-5">
          <div className="flex items-center gap-2 font-bold text-red-800">
            <ShieldAlert className="size-5" /> Generation error
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_22rem]">
        <div className="space-y-5">
          <AdminSection title="Request & generation context">
            <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <Fact
                label="Request ID"
                value={data.job.custom_lesson_request_id}
              />
              ...
              <Fact
                label="JLPT"
                value={data.request?.jlpt_level || lesson?.jlpt_level || "—"}
              />
              <Fact
                label="Created"
                value={new Date(data.job.created_at).toLocaleString()}
              />
              <Fact
                label="Source lesson"
                value={
                  data.job.source_lesson_id || lesson?.source_lesson_id || "None"
                }
              />
            </dl>
          </AdminSection>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex flex-wrap justify-between gap-2">
                        ...
                        <AdminStatus>{check.severity}</AdminStatus>
                      </div>
          ...
          {!categories.length && (
            <AdminSection title="Validation checks">
              <p className="py-8 text-center text-sm text-slate-500">
                No persisted validation checks exist for this generated lesson.
              </p>
            </AdminSection>
          )}
        ...
        <aside className="space-y-5">
          <AdminSection
            ...
            description="These operations write to Supabase and are audit logged."
          >
            <label className="block">
              <span className="mb-2 block text-sm font-bold">Reviewer note</span>
              <textarea
                value={note}
                onChange={(event) => setNote(event.target.value)}
                rows={4}
                className="admin-input min-h-28 py-3"
                placeholder="Optional reason, correction note, or publish summary"
              />
            </label>
            <div className="mt-3 space-y-2">
              <Button
                ...
                disabled={working || currentStatus === "published"}
              ...
              <Button
                ...
                disabled={
                  working || !["approved", "published"].includes(currentStatus)
                }
          ...
          <AdminSection title="Regeneration">
            <div className="rounded-xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">
              Section-level regeneration is unavailable because there is no
              server-side mutation that can safely preserve approved sections,
              replace only the requested region, and audit the change.
            </div>
            <Link
              href="/admin/generated"
              className="mt-3 inline-flex items-center gap-2 text-sm font-bold text-teal-700 hover:underline"
            >
              <Sparkles className="size-4" /> Return to generation queue
            </Link>
          </AdminSection>

          {data.job.source_lesson_id && (
            <AdminSection title="Variation source">
              <Link
                href={`/admin/lessons/${data.job.source_lesson_id}`}
                className="break-all text-sm font-bold text-teal-700 hover:underline"
              >
                {data.job.source_lesson_id}
              </Link>
            </AdminSection>
