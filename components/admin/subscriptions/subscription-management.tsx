...
export function SubscriptionManagement() {
  ...
  const records: SubscriptionRecord[] = backendMode
    ? backendRows.map((row) => ({
        id: row.id,
        userId: row.user_id,
  ...
  async function update(recordId: string, nextStatus: SubscriptionRecordStatus, nextPlan?: SubscriptionPlan) {
    setError("");
    setMessage("");
    if (!backendMode) {
      updateLocal(recordId, nextStatus, nextPlan);
      return;
    }
  ...
  const attention = records.filter((record) => record.status === "failed" || record.status === "cancelled").length;
  ...
  return (
    <>
      <AdminPageHeader
        ...
        description={backendMode ? "Live plan and subscription-state records from Supabase. Billing is not connected to Stripe, so revenue is intentionally not estimated here." : "Local subscription simulation for demo mode."}
        actions={backendMode ? <Button type="button" variant="secondary" className="rounded-xl" disabled={loading} onClick={() => void load()}><RefreshCw className={`size-4 ${loading ? "animate-spin" : ""}`} />Refresh</Button> : undefined}
      />
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <AdminStatCard label="Cancelled / past due" value={attention} tone={attention ? "red" : "teal"} detail={backendMode ? "No revenue inferred without billing facts" : "Demo states"} />
      </div>
      ...
      <div className="mt-5">
        {loading && !records.length ? <div className="h-64 animate-pulse rounded-2xl bg-slate-100" /> : <AdminTable caption="Subscription records" headers={["User", "Plan", "Billing", "Status", "Started", "Renewal", "Cancellation", "Payment state", "Actions"]} rows={visible.map((record) => ({
          id: record.id,
          cells: [
            backendMode ? <Link key="user" href={`/admin/users/${record.userId}`} className="font-bold text-teal-700">{record.userName}</Link> : record.userName,
            <select key="plan" value={record.plan} onChange={(event) => void update(record.id, "active", event.target.value as SubscriptionPlan)} className="rounded-lg border border-slate-200 px-2 py-1 text-xs"><option>free</option><option>premium</option></select>,
            record.billingInterval,
            <AdminStatus key="status">{record.status}</AdminStatus>,
            record.startDate,
            record.renewalDate ?? "—",
            record.cancellationDate ?? "—",
            <AdminStatus key="pay">{record.paymentStatus}</AdminStatus>,
            <div key="actions" className="flex flex-wrap gap-1"><Small onClick={() => void update(record.id, record.status === "cancelled" ? "active" : "cancelled")}>{record.status === "cancelled" ? "Reactivate" : "Cancel"}</Small><Small onClick={() => void update(record.id, "trial", "premium")}>Mark trial</Small>{record.status === "failed" && <Small onClick={() => void update(record.id, "active")}><RefreshCw className="size-3" />Mark resolved</Small>}</div>,
          ],
        }))} />}
      </div>
      {backendMode && <p className="mt-4 text-xs leading-5 text-slate-500">Payment state is still the application’s stored mock-payment field. No charge, refund, invoice, or payment-provider action occurs from this screen.</p>}
    </>
  );
}

function Small({ children, onClick }: { children: React.ReactNode; onClick: () => void }) {
  return <Button type="button" variant="secondary" className="min-h-9 rounded-lg px-3 text-xs" onClick={onClick}>{children}</Button>;
}
