    ...
    const items = lessons
      ...
      .filter(
        (lesson) =>
          ...
          !(lesson.readingQuestions?.length) ||
    ...
    <>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-sm">
          <CheckFilter
            ...
            label="Missing required content"
          />
        </div>
      </div>
      ...
      {selected.length > 0 && (
        <div className="mt-4 flex flex-wrap items-center gap-2 rounded-2xl border border-teal-200 bg-teal-50 p-3">
          ...
          <Button type="button" variant="secondary" className="min-h-10 rounded-lg px-4" onClick={() => void changeStatuses(selectedLessons, "published")}>Publish</Button>
          <Button type="button" variant="secondary" className="min-h-10 rounded-lg px-4" onClick={() => void changeStatuses(selectedLessons, "draft")}>Unpublish</Button>
          <Button type="button" variant="secondary" className="min-h-10 rounded-lg px-4" onClick={() => void changeStatuses(selectedLessons, "archived")}>Archive</Button>
          <Button type="button" variant="secondary" className="min-h-10 rounded-lg px-4" onClick={() => exportLessons(selectedLessons)}><Download className="size-4" /> Export</Button>
        </div>
      )}
      ...
      <div className="mt-5 hidden lg:block">
        {loading && !lessons.length ? (
          ...
          <AdminTable
            ...
            headers={["Select", "Lesson", "Level", "Topic", "Source", "Status", "Duration", "Grammar", "Kanji", "Actions"]}
            rows={visible.map((lesson) => ({
              ...
              cells: [
                ...
                lesson.kanji.length,
                <LessonActions key="actions" lesson={lesson} onStatus={changeStatus} />,
              ],
            }))}
          />
        ) : (
          <AdminEmptyState title="No lessons found" description="Adjust the search or filters to restore persisted lesson records." />
        )}
      </div>
        ...
        {visible.map((lesson) => (
          <article key={lesson.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div><Link href={`/admin/lessons/${lesson.id}`} className="font-bold text-slate-900">{lesson.title}</Link><p className="mt-1 text-sm text-slate-500">{lesson.japaneseTitle}</p></div>
              <AdminStatus>{lesson.status}</AdminStatus>
            </div>
            <p className="mt-4 text-sm text-slate-600">{lesson.level} · {lesson.topic} · {lesson.durationMinutes} min</p>
            <div className="mt-4"><LessonActions lesson={lesson} onStatus={changeStatus} /></div>
          </article>
        ))}
      </div>

      <div className="mt-5 flex items-center justify-between text-sm text-slate-500">
        <span>{filtered.length} records · page {currentPage} of {pages}</span>
        <div className="flex gap-2">
          <Button type="button" variant="secondary" className="min-h-10 rounded-lg px-4" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Previous</Button>
          <Button type="button" variant="secondary" className="min-h-10 rounded-lg px-4" disabled={page >= pages} onClick={() => setPage((value) => value + 1)}>Next</Button>
        </div>
      </div>
    </>
  );
...
function Filter({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) {
  return (
    <label>
      ...
      <select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm capitalize outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-100">
...
function LessonActions({ lesson, onStatus }: { lesson: LessonPackage; onStatus: (lesson: LessonPackage, status: LessonStatus) => void }) {
  return (
    <div className="flex min-w-max gap-1">
      <ActionLink href={`/lesson/${lesson.id}/preview`} label="Preview"><Eye className="size-4" /></ActionLink>
      <ActionLink href={`/admin/lessons/${lesson.id}/edit`} label="Edit"><Pencil className="size-4" /></ActionLink>
      <ActionLink href={lesson.source.includes("generated") ? `/admin/generated/${lesson.id}` : `/admin/lessons/${lesson.id}`} label="Validate"><FileCheck2 className="size-4" /></ActionLink>
      <ActionButton label={lesson.status === "published" ? "Unpublish" : "Publish"} onClick={() => onStatus(lesson, lesson.status === "published" ? "draft" : "published")}><FileCheck2 className="size-4" /></ActionButton>
      <ActionButton label="Archive" onClick={() => onStatus(lesson, "archived")}><Archive className="size-4" /></ActionButton>
    </div>
  );
}
...
function ActionButton({ label, onClick, children }: { label: string; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" title={label} aria-label={`${label} lesson`} onClick={onClick} className="grid size-9 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-teal-700">{children}</button>;
}
