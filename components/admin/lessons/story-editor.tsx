  ...
  return (
    <EditorPanel title="Story" description="Edit, reorder, and link the contextual source used by later phases." error={errors.story ?? errors.storyLines}>
      <div className="space-y-4">
        {lesson.story.map((line, index) => <article key={line.id} className="rounded-xl border border-slate-200 p-4"><div className="mb-4 flex items-center justify-between"><strong className="text-sm">Line {index + 1} · {line.id}</strong><div className="flex gap-1"><IconButton label="Move line up" disabled={index === 0} onClick={() => move(index, -1)}><ArrowUp className="size-4" /></IconButton><IconButton label="Move line down" disabled={index === lesson.story.length - 1} onClick={() => move(index, 1)}><ArrowDown className="size-4" /></IconButton><IconButton label="Remove line" onClick={() => onChange({ ...lesson, story: lesson.story.filter((_, itemIndex) => itemIndex !== index) })} danger><Trash2 className="size-4" /></IconButton></div></div><div className="grid gap-3 sm:grid-cols-2"><EditorField label="Japanese text"><textarea value={line.japanese} onChange={(event) => update(index, { ...line, japanese: event.target.value })} className={`${inputClass} min-h-20 py-3`} lang="ja" /></EditorField><EditorField label="English text"><textarea value={line.english} onChange={(event) => update(index, { ...line, english: event.target.value })} className={`${inputClass} min-h-20 py-3`} /></EditorField><EditorField label="Vocabulary tokens"><input value={line.tappableTerms.join(", ")} onChange={(event) => update(index, { ...line, tappableTerms: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) })} className={inputClass} /></EditorField><EditorField label="Image position / asset ID"><input value={line.imageId ?? ""} onChange={(event) => update(index, { ...line, imageId: event.target.value || undefined })} className={inputClass} placeholder="img_asset_id" /></EditorField><EditorField label="Mock audio asset"><input value={line.audioAssetId ?? ""} onChange={(event) => update(index, { ...line, audioAssetId: event.target.value || undefined })} className={inputClass} placeholder="audio_asset_id" /></EditorField></div></article>)}
      </div>
