...
export function LessonEditor({ lessonId }: { lessonId: string }) {
  ...
  function save(status: "draft" | "published") {
    ...
    setShowErrors(false);
    setMessage(status === "published" ? "Lesson published. The learner library now reads this canonical version." : "Draft saved locally.");
  ...
  return (
    <>
      ...
      <div className="sticky bottom-4 z-20 mt-7 flex flex-wrap items-center gap-2 rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-xl backdrop-blur">
        <Button type="button" variant="secondary" className="rounded-xl" onClick={() => save("draft")}><Save className="size-4" /> Save draft</Button>
        <Button type="button" className="rounded-xl" onClick={() => save("published")}><Send className="size-4" /> Publish</Button>
