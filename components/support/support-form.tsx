...
export function SupportForm({
  ...
  if (success) {
    return <div className="rounded-3xl border border-positive-border bg-positive-surface p-7 text-center" role="status"><span className="mx-auto grid size-14 place-items-center rounded-2xl bg-positive text-white"><Check className="size-6" aria-hidden="true" /></span><h3 className="mt-5 text-xl font-semibold">Request saved.</h3><p className="mt-2 text-sm text-muted">{getBackendMode() === "supabase" ? "Your ticket is ready for the support team." : "Demo mode recorded your request on this device."}</p><Button type="button" variant="secondary" className="mt-5" onClick={() => setSuccess(false)}>Send another request</Button></div>;
  }
  ...
  return (
    <form className="space-y-4" onSubmit={submit}>
      ...
      <label className="block"><span className="mb-2 block text-sm font-semibold">Email</span><input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="form-input" placeholder="you@example.com" autoComplete="email" /></label>
      ...
      <Button type="submit" disabled={loading} aria-busy={loading} className="w-full"><Send className="size-4" aria-hidden="true" /> {loading ? "Sending…" : "Save support request"}</Button>
    </form>
  );
}
