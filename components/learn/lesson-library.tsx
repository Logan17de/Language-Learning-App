...
import { ArrowRight, CheckCircle2, LoaderCircle, Play, RotateCcw } from "lucide-react";
...
export function LessonLibrary() {
  ...
  const assignedLesson = useMemo(() => {
    const availableLessons = backendMode === "supabase" ? backendLessons : allDemoLessons;
    if (activeSession) {
      const activeLesson = availableLessons.find((lesson) => lesson.id === activeSession.lessonId);
  ...
  }, [activeSession, allDemoLessons, backendLessons, backendMode, completedIds, onboarding.interests, onboarding.level, premium, user.id, user.level]);
  ...
  async function startNewLesson() {
    ...
    const next = selectNextLesson({
      ...
      interests: onboarding.interests,
      premium,
      excludedLessonIds: Array.from(new Set([...completedIds, assignedLesson.id])),
    ...
    <div className="mx-auto max-w-4xl px-5 py-7 sm:px-8 sm:py-10">
      <header className="text-center">
        ...
        <p className="mx-auto mt-3 max-w-2xl leading-7 text-stone-500">
          {isResuming
            ? "Resume from your checkpoint, or start a different lesson. Scores from everything you already attempted stay saved."
            : "AIko has selected one lesson for your level. Its title and topic remain hidden until you start."}
        </p>
      </header>

      <section className="mt-9">
        {loading ? (
          <div className="h-80 animate-pulse rounded-4xl bg-moss-50" aria-label="Preparing your lesson" />
        ) : assignedLesson ? (
          <Card className="relative overflow-hidden !bg-moss-900 p-8 text-center text-white sm:p-12">
            ...
            <div className="relative">
              <span className="mx-auto grid size-14 place-items-center rounded-2xl bg-white/10 text-persimmon-400">
                {isResuming ? <RotateCcw className="size-6" /> : <Play className="size-6" />}
              </span>
              <p className="mt-7 text-xs font-bold uppercase tracking-[.18em] text-moss-200">{assignedLesson.level} lesson</p>
              <h2 className="mt-3 text-3xl font-semibold">{isResuming ? "Lesson in progress" : "Private lesson reveal"}</h2>
              <p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-white/65">
                {isResuming ? "Your last activity, partial mastery evidence, and elapsed time are preserved." : "No title, topic, story, or vocabulary is shown before you begin."}
              </p>
              <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
                <ButtonLink href={`/lesson/${assignedLesson.id}/play`} className="bg-persimmon-500 px-8 hover:bg-persimmon-600">
                  {isResuming ? "Resume old lesson" : "Start lesson"}<ArrowRight className="size-4" />
                </ButtonLink>
                {isResuming && (
                  <Button type="button" variant="secondary" disabled={backendAssigning} onClick={() => void startNewLesson()} className="border-white/20 bg-white/10 px-8 text-white hover:bg-white/15">
                    {backendAssigning ? <LoaderCircle className="size-4 animate-spin" /> : <Play className="size-4" />}
                    {backendAssigning ? "Finding lesson…" : "Start new lesson"}
                  </Button>
                )}
              </div>
              {backendError && <p role="alert" className="mx-auto mt-4 max-w-lg text-sm font-semibold text-persimmon-200">{backendError}</p>}
            </div>
          </Card>
        ) : (
          <Card className="grid min-h-72 place-items-center border-dashed text-center">
            <div>
              ...
              <h2 className="mt-5 text-xl font-semibold">Every available lesson is complete.</h2>
              <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-stone-500">{backendError || "AIko will prepare another level-matched lesson when new content is available."}</p>
            </div>
          </Card>
        )}
      </section>
