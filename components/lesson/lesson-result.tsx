...
export function LessonResult({ lesson, result }: { lesson: LessonPackage; result: LessonCompletionResult }) {
  const rewardLessonCompletion = useAppStore((state) => state.rewardLessonCompletion);
  const rewarded = useAppStore((state) => state.lessonSessions[lesson.id]?.rewarded ?? false);
    ...
    <main className="relative min-h-screen overflow-hidden bg-paper px-5 py-10 sm:px-8 sm:py-16">
      <div className="pointer-events-none absolute inset-0 motion-reduce:hidden" aria-hidden="true">
        {["left-[12%] top-16", "left-[25%] top-32", "right-[18%] top-20", "right-[30%] top-44"].map((position, index) => (
          <span key={position} className={`absolute ${position} animate-float-slow ${index % 2 ? "text-moss-500" : "text-persimmon-400"}`}>
            {index % 2 ? <Star className="size-5 fill-current" /> : <Sparkles className="size-6" />}
          </span>
        ))}
      </div>
      <div className="relative mx-auto max-w-4xl">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-3">
          <ResultStat icon={Trophy} value={`+${result.xpGained}`} label="XP gained" />
          <ResultStat icon={Flame} value={`${Math.max(streak, 13)} days`} label="Current streak" />
          <ResultStat icon={Clock3} value={`${result.durationMinutes} min`} label="Lesson duration" />
        </div>
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-2">
          ...
          <Card className="p-7">
            <h2 className="flex items-center gap-2 font-semibold"><Volume2 className="size-5 text-persimmon-500" /> Skill changes</h2>
            <div className="mt-6 space-y-5">
              <SkillChange label="Kanji recognition" value={72} change={result.recognitionChange} />
              <SkillChange label="Pronunciation confidence" value={66} change={result.pronunciationChange} />
              <SkillChange label="Grammar understanding" value={74} change={result.grammarUnderstandingChange} />
              <SkillChange label="Grammar production" value={59} change={result.grammarProductionChange} />
            </div>
          </Card>
        </div>
...
function SkillChange({ label, value, change }: { label: string; value: number; change: number }) {
  return (
    <div>
      <div className="mb-2 flex items-center justify-between text-sm"><span className="font-medium">{label}</span><span className="font-semibold text-moss-700">+{change}</span></div>
