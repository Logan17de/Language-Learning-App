-- Phase-atomic mastery and rollout compatibility ship as one pending migration.
-- Neither half may be applied independently: this file installs the canonical
-- engine, historical 7-of-10/13 compatibility, cached-client bridge, resume
-- semantics, and mastery-ledger hardening as a single migration unit.
...
-- Rollout and historical-data compatibility are deliberately in this same file
-- so the database cannot record the canonical engine migration without also
-- installing the cached-client bridge and seven-activity historical validator.
...
-- Final guardrails cover the entire consolidated rollout state.
do $$
begin
